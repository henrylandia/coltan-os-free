...
const { classifyAlert } = require('./suricata.service')
...
async function collectSuricataAlerts() {
  ...
  try {
    ...
    const stmt = db.prepare(`
      INSERT INTO attack_log (src_ip, country, country_code, city, isp, attack_type, threat_key, threat_label, threat_icon, severity, signature, proto, dest_port, blocked, detected_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
    ...
    for (const line of lines) {
      try {
        ...
        const classification = classifyAlert(event.alert?.category)
        stmt.run(
          srcIP,
          geo.country || null,
