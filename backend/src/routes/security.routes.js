...
const { getDNSStats,
  ...
  checkListHealth, checkAllListsHealth,
...
async function securityRoutes(fastify, options) {
  ...
  // ── Gestion de listas del DNS Blocker ────────────────────────────────────
  fastify.get('/api/security/dns/lists', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    return await getAvailableLists()
  })
  fastify.post('/api/security/dns/lists/:id/toggle', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    const { enabled } = req.body
    return await toggleList(req.params.id, enabled)
  })
  fastify.post('/api/security/dns/lists/custom', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    const { name, url, category, format } = req.body
    return await addCustomList(name, url, category, format)
  })
  fastify.delete('/api/security/dns/lists/custom/:id', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    return await removeCustomList(req.params.id)
  })
  fastify.post('/api/security/dns/lists/:id/check', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    return await checkListHealth(req.params.id)
  })
  fastify.post('/api/security/dns/lists/check-all', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    return await checkAllListsHealth()
  })
  fastify.post('/api/security/dns/update-now', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    return await performBlocklistUpdate('manual')
  })
  fastify.post('/api/security/dns/auto-update', { onRequest: [fastify.authenticate] }, async (req, reply) => {
    return await setAutoUpdate(req.body.enabled)
  })
}
